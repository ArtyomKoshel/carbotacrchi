...
<p class="text-sm text-gray-500 mb-6">
  Изменения применяются автоматически в течение 1 минуты (горячая перезагрузка). Формат: <code class="text-gray-400">interval:60</code> или <code class="text-gray-400">cron:0 * * * *</code>
</p>
  ...
  <div class="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
    ...
    <form method="POST"
      ...
      <div class="flex flex-col gap-1">
        <label class="text-xs text-gray-500">Включено</label>
        <select name="enabled" class="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white">
          <option value="1" {{ $s?->enabled ? 'selected' : '' }}>Да</option>
          <option value="0" {{ ($s && !$s->enabled) ? 'selected' : '' }}>Нет</option>
        </select>
      </div>

      <div class="flex flex-col gap-1">
        <label class="text-xs text-gray-500">Расписание</label>
      ...
      <div class="flex flex-col gap-1">
        <label class="text-xs text-gray-500">Интервал (мин)</label>
      ...
      <div class="flex flex-col gap-1">
        <label class="text-xs text-gray-500">Макс. страниц <span class="text-gray-600">(0=все)</span></label>
      ...
      <div class="flex flex-col gap-1">
        <label class="text-xs text-gray-500">Фильтр марки</label>
      ...
      <div class="col-span-full flex items-center gap-3">
        ...
        @if($s)
        <span class="text-xs text-gray-600">Обновлено: {{ $s->updated_at->diffForHumans() }}</span>
        @endif
      </div>
    </form>
  </div>
